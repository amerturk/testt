...
namespace Signbook.Views
{
    ...
    public partial class SignupJordanPage : BaseContentPage
    {
        
        public SignupJordanPage()
        {
            InitializeComponent();
            ...
            async void RegisterUser(object sender, EventArgs args)
        {
            ...
            if (Name != null && Password != null && UserEmail != null)
            {



                if (Name.Length <= 0 || Password.Length <= 0 || UserEmail.Length <= 0 )
                {
                    validfields = false;
                }
                if (Name.Length > 50 || Password.Length > 50 || UserEmail.Length > 50 )
                {
                    validfields = false;
                }
        ...
        public void register(string nName, string pPassword,string eEmail, string pPhone, string idNumber)
        {
            ...
            var Registrationurl = string.Format("http://jodata.signbook.co/api/SitesApis/Register?UserName={0}&Password={1}&Email={2}&FullName={0}&phone={3}&DisabilityType=0&Status=0&Balance=0&UserIDNumber={4}", nName, pPassword, eEmail, pPhone, idNumber);
